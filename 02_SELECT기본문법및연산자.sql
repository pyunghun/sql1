...
SELECT
      ...
      , SALARY
  FROM EMPLOYEE
  WHERE SALARY >= 3000000; --SALARY 와 3000000의 순서가 달라져도 상관없으나 X
...
SELECT
       EMP_NAME AS 이름
     , SALARY * 12 AS "1년급여" --산술 결과를 출력 가능하다. 결과집합(RESULT SET)
...
--연결연산자를 이용하여 여러 컬럼의 값을 하나의 컬럼의 값인 것처럼 연결할 수 있다.
SELECT
...
--근무여부를 별침으로 하여 재직중이라는 문자열을 결과집합에 포함하여 조회
...
2. 연결연산자
3. 비교연산자
4. ISNULL / IS NOT NULL / LIKE / NOT LIKE / IN / NOT IN
5. BETWEEN
6. NOT
7. AND
8. OR
